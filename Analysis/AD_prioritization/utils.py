...
def compute_effects(snpeffects,n_snps,all_models, nfeatures,maxshift=800,batchSize=500):
    ...
    for i in range(int( (n_snps - 1) / batchSize) + 1):
        ...
        diff = reduce(lambda x, y: x + y,
                        [np.tile(np.asarray(snpeffects[j][i * batchSize:(i + 1) * batchSize, :]), 10)for j in range(len(dists))] # (n_dists, n_snps, 10)
